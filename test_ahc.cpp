...
// define the design_name to build, i.e. 3_13 for 3 integer bits, 13 decimal bits
#if USE_FLOAT
	const std::string design_name = "\\float";
#else
	const std::string design_name = "\\" + to_string(intBits) + "_" +to_string(fracBits); // "\\4_20";
#endif

// define a list of SNR values to solve
#define USE_HIGH_SNR 0 // 1 for high SNR like 10000, 0 for low SNR
#if USE_HIGH_SNR
	int SNR_list[] = {10000};
	const int numSNR = 1;
	const int numProblems = 1; // use check if bestSpins[i] + bestSpins[i+2*N_t] are all zero
#else
	int SNR_list[] = {0, 5, 10, 15, 20, 25, 30}; // {0, 5, 10, 15, 20, 25, 30};
	const int numSNR = sizeof(SNR_list)/sizeof(SNR_list[0]); // calculate the number of elements in the SNR_list
	const int numProblems = 100; // use Python to draw BER
#endif
...
std::string fullPathTo_hls_solved_solutions = fullPathTo_Tb_data + "\\HLS_solved_solutions\\Nt16_Nr16_16QAM" + design_name;
...
int main()
{
  ...
  // load the error_var_init
  ifstream error_var_init_file( fullPathTo_error_init );
  if ( !error_var_init_file.is_open() ) {
    cout << "Error opening x_init file!" << endl;
    return 1;
  }

  // Define a 2D array to store all 20 error_var_init_file arrays
  for ( int array_idx = 0; array_idx < num_anneals; array_idx++ ) {
    for ( int i = 0; i < matrix_size; i++ ) {
      if ( !( error_var_init_file >> error_var_init_arrays[array_idx][i] ) ) {
        cout << "Error reading x_init file!" << endl;
        return 1;
      }
    }
  }
  error_var_init_file.close();
  ...
  for (int SNR_idx = 0; SNR_idx < numSNR; SNR_idx++){
    std::string SNR = to_string(SNR_list[SNR_idx]);
